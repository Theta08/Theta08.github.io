...
### 플레이어
플레이어는 `PlayerController.cs`를 가진다. 움직임은 마우스 클릭으로 움직인다. 메니저에서`InputManager`를 호출해 마우스 클릭 엑션상태를 가져온다. `OnMouseEvent_IdleRun`함수 실행한다. 카메라기준 마우스 클릭한 지점과 플레이어 지점 거리를 계산해서 이동한다.
